...
export const _routes = {
  ...
  public: {
    ...
    submissionSuccess: `${_paths.public}/${_paths.publicPath.submissionSuccess}`,
  },
  security: {
  ...
  clinical: {
    patients: {
      ...
      view(patientId: number): string {
        return `${_paths.clinical}/${_paths.clinicalPath.patients.view.replace(
          ':patientId',
          patientId.toString()
        )}`;
      },
    ...
    encounters: {
      ...
      view(patientId: number, encounterId: number): string {
        return `${_paths.clinical}/${_paths.clinicalPath.encounters.view
    ...
    cohorts: {
      ...
      view(cohortGroupId: number): string {
        return `${_paths.clinical}/${_paths.clinicalPath.cohorts.view.replace(
          ':cohortGroupId',
          cohortGroupId.toString()
        )}`;
      },
    },
    forms: {
      ...
      listForm(type: string): string {
        return `${_paths.clinical}/${_paths.clinicalPath.forms.list.replace(
          ':type',
          type
        )}`;
      },
      synchroniseForm(type: string): string {
        return `${
          _paths.clinical
        }/${_paths.clinicalPath.forms.synchronise.replace(':type', type)}`;
      },
      viewFormA(formId: number): string {
        return `${_paths.clinical}/${_paths.clinicalPath.forms.forma.replace(
          ':formId',
          formId.toString()
        )}`;
      },
      viewFormB(formId: number): string {
        return `${_paths.clinical}/${_paths.clinicalPath.forms.formb.replace(
          ':formId',
          formId.toString()
        )}`;
      },
      viewFormC(formId: number): string {
        return `${_paths.clinical}/${_paths.clinicalPath.forms.formc.replace(
          ':formId',
          formId.toString()
        )}`;
      },
      viewFormADR(formId: number): string {
        return `${_paths.clinical}/${_paths.clinicalPath.forms.formadr.replace(
          ':formId',
          formId.toString()
        )}`;
      },
  ...
  analytical: {
    ...
    reports: {
      search(workFlowId: string): string {
        return `${
          _paths.analytical
        }/${_paths.analyticalPath.reports.search.replace(':wuid', workFlowId)}`;
      },
      searchByQualifiedName(workFlowId: string, qualifiedName: string): string {
        return `${
          _paths.analytical
        }/${_paths.analyticalPath.reports.searchqualified
          .replace(':wuid', workFlowId)
          .replace(':qualifiedName', qualifiedName)}`;
      },
      activity(workFlowId: string, reportInstanceId: number): string {
        return `${_paths.analytical}/${_paths.analyticalPath.reports.activity
          .replace(':wuid', workFlowId)
          .replace(':reportinstanceid', reportInstanceId.toString())}`;
      },
      clinical(patientId: number, patientClinicalEventId: number): string {
        return `${_paths.analytical}/${_paths.analyticalPath.reports.clinical
          .replace(':patientId', patientId.toString())
          .replace(':clinicalEventId', patientClinicalEventId.toString())}`;
      },
      task(workFlowId: string, reportInstanceId: number): string {
        return `${_paths.analytical}/${_paths.analyticalPath.reports.task
          .replace(':wuid', workFlowId)
          .replace(':reportinstanceid', reportInstanceId.toString())}`;
      },
    },
  ...
  information: {
    view(id: number): string {
      return `${_paths.information}/${_paths.informationPath.view.replace(
        ':id',
        id.toString()
      )}`;
    },
  ...
  administration: {
    ...
    work: {
      ...
      datasetcategoryView(datasetId: number): string {
        return `${
          _paths.administration
        }/${_paths.administrationPath.work.datasetcategory.replace(
          ':datasetid',
          datasetId.toString()
        )}`;
      },
      datasetcategoryElementView(
        datasetId: number,
        datasetCategoryId: number
      ): string {
        return `${
          _paths.administration
        }/${_paths.administrationPath.work.datasetcategoryelement
          .replace(':datasetid', datasetId.toString())
          .replace(':datasetcategoryid', datasetCategoryId.toString())}`;
      },
    },
  },
};
