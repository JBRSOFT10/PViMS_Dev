...
export class SpontaneousTablePopupComponent
  ...
  implements OnInit, AfterViewInit
{
  ...
  ngOnInit(): void {
    ...
    self.data.datasetElementSubs.forEach(element => {
      ...
      elements.addControl(
        element.datasetElementSubId.toString(),
  ...
  handleUnknown() {
    ...
    const ix = 0;
    let count=0;
    let elements = self.itemForm.controls.elements as FormGroup;
    console.log(elements);
    console.log(elements.get(`${ix + 1}`).value);
    console.log(this.itemForm.value);
    console.log('begin value');
    for (let index = 0; index < self.data.datasetElementSubs.length; index++) {

      if(elements.get(`${index + 1}`).value==null){
        elements.get(`${index + 1}`).setValue('N/A');
        count=count+1;
      }
