...
import {
  ProgressStatusEnum,
  ProgressStatus,
} from 'app/shared/models/program-status.model';
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  initialiseReport(): void {
    ...
    self.qualifiedName =
      self._activatedRoute.snapshot.paramMap.get('qualifiedName');

    self.viewModelForm = self._formBuilder.group({
      qualifiedName: [self.viewModel.qualifiedName || ''],
      searchFrom: [
        self.viewModel.searchFrom || moment().subtract(3, 'months'),
        Validators.required,
      ],
  ...
  ngAfterViewInit(): void {
    let self = this;
    self.viewModel.mainGrid
      .setupAdvance(null, null, self.mainGridPaginator)
      .subscribe(() => {
        self.loadGrid();
      });
  ...
  setupTable() {
    ...
    if (this.currentScreenWidth === 'sm') {
      this.viewModel.mainGrid.updateDisplayedColumns([
        'patient-identifier',
        'status',
        'actions',
      ]);
  ...
  downloadAttachment(data: any = {}): void {
    ...
    this.reportInstanceService
      .downloadAttachment(
        self.workflowId,
        data.id,
        data.activityExecutionStatusEventId,
        data.attachmentId
      )
      .subscribe(
        data => {
  ...
  downloadSummary(data: any = {}): void {
    ...
    this.reportInstanceService
      .downloadSummary(this.workflowId, data.id)
      .subscribe(
        data => {
          switch (data.type) {
            case HttpEventType.DownloadProgress:
  ...
  selectActivity(activity: string): void {
    ...
    if (activity == 'ADRM') {
      self.viewModel.mainGrid.updateDisplayedColumns([
        'patient-identifier',
        'created',
        'adverse-event',
        'gender',
      ...
      self.viewModel.mainGrid.updateDisplayedColumns([
        ...
        'gender',
        // 'brandTradeName',
        'genericNameWithStrength',
        'dosageFrom',
        'frequencyDailyDose',
        'indication',
        'medicationstartdate',
        'eventstartdate',
        'describeeventincludingrelevanttestsandlaboratoryresults',
        'adrmStatus',
        'opinionTsc',
        'opinionAdrac',
      ]);
  ...
  loadGrid(): void {
    ...
    switch (self.viewModel.searchContext) {
      case 'New':
        self.reportInstanceService
          .getNewReportInstancesByDetail(
            self.workflowId,
            self.viewModel.mainGrid.customFilterModel(self.viewModelForm.value)
          )
      ...
      case 'Active':
        if (self.viewModelForm.value.qualifiedName === 'LineReport') {
          self.reportInstanceService
            .getLineReport(
              self.viewModel.mainGrid.customFilterModel(
                self.viewModelForm.value
              )
            )
            .pipe(takeUntil(self._unsubscribeAll))
            .pipe(finalize(() => self.setBusy(false)))
            .subscribe(
              ...
              error => {
                this.handleError(
                  error,
                  'Error getting report instances by activity'
                );
              }
            );
        } else {
          ...
          self.reportInstanceService
            ...
            .subscribe(
              ...
              error => {
                this.handleError(
                  error,
                  'Error searching for report instances by date'
                );
              }
            );
        }
          ...
          self.reportInstanceService
            ...
            .subscribe(
              ...
              error => {
                this.handleError(
                  error,
                  'Error searching for report instances by term'
                );
              }
            );
        }
        ...
        result => {
          const rows = result.value.map(item => {
            return {
              ...
              age: item.age,
              gender: item.gender,
              genericName: item.genericNameWithStrength,
              dosageFrom: item.dosageFrom,
              frequency: item.frequencyDailyDose,
              medStartDate: item.medicationstartdate,
              eventstartdate: item.eventstartdate,
              indication: item.indication,
              describeEvent:
                item.describeeventincludingrelevanttestsandlaboratoryresults,
              adrmStatus: item.adrmStatus,
              opinionTsc: item.opinionTsc,
              opinionAdrac: item.opinionAdrac,
            };
          });
          const worksheet = XLSX.utils.json_to_sheet(rows);
          const workbook = XLSX.utils.book_new();

          XLSX.utils.book_append_sheet(workbook, worksheet, 'Line Reports');
          ...
          XLSX.writeFile(workbook, 'LineReport.xlsx', { compression: true });
        },
        error => {
  ...
  detailActivity(model: GridRecordModel = null): void {
    ...
    self._router.navigate([
      _routes.analytical.reports.activity(
        self.workflowId,
        model ? model.id : 0
      ),
    ]);
  ...
  detailTask(model: GridRecordModel = null): void {
    let self = this;
    self._router.navigate([
      _routes.analytical.reports.task(self.workflowId, model ? model.id : 0),
    ]);
  }
  ...
  detailPatient(model: GridRecordModel = null): void {
    let self = this;
    self._router.navigate([
      _routes.clinical.patients.view(model ? model.patientId : 0),
    ]);
  }
  ...
  detailClinical(patientId: number, patientClinicalEventId: number) {
    let self = this;
    self._router.navigate([
      _routes.analytical.reports.clinical(patientId, patientClinicalEventId),
    ]);
  }
  ...
  createE2B(data: any = {}) {
    ...
    self.reportInstanceService
      .createE2B(self.workflowId, data.id)
      .pipe(finalize(() => self.setBusy(false)))
      .subscribe(
        result => {
  ...
  private pepareChartSeries() {
    ...
    const aesi: SeriesValueListModel = {
      name: 'AESI',
      series: self.prepareSeriesItem(
        self.viewModel.workFlowSummary.classifications.find(
          c => c.classification == 'AESI'
        )
      ),
    };
    ...
    const significant: SeriesValueListModel = {
      ...
      series: self.prepareSeriesItem(
        self.viewModel.workFlowSummary.classifications.find(
          c => c.classification == 'Clinically Significant'
        )
      ),
    };
...
class ViewModel {
  mainGrid: GridModel<GridRecordModel> = new GridModel<GridRecordModel>([
    'patient-identifier',
    'created',
    'medication-summary',
    'adverse-event',
