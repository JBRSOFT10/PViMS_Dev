...
namespace PVIMS.Services
{
    public class WorkFlowService : IWorkFlowService 
    {
        ...
        public async Task<ReportInstance> CreateWorkFlowInstanceAsync(string workFlowName, Guid contextGuid, string patientIdentifier, string sourceIdentifier,string gender, string organization, string reportingType, string facilityIdentifier)
        {
            ...
            {
                throw new ArgumentException($"'{nameof(reportingType)}' cannot be null or whitespace.", nameof(reportingType));
            }*/
            ...
            if (reportInstance == null)
            {
                reportInstance = new ReportInstance(workFlow, currentUser, contextGuid,patientIdentifier, sourceIdentifier,gender,organization,reportingType,facilityIdentifier);
                await _reportInstanceRepository.SaveAsync(reportInstance);

                reportInstance.SetSystemIdentifier();
                reportInstance.SetPatientIdentifier();
        ...
        public async Task<ActivityExecutionStatusEvent> ExecuteActivityAsync(Guid contextGuid, string newExecutionStatus, string comments, DateTime? contextDate, string contextCode)
        {
            ...
            var activityExecutionStatusEvent = reportInstanceFromRepo.ExecuteNewEventForActivity(newActivityExecutionStatus, currentUser, comments, contextDate, contextCode,reportInstanceFromRepo.ReportClassificationId);
