...
namespace PVIMS.API.Infrastructure
{
    public class PVIMSContextSeed
    {
        ...
        private static IEnumerable<Activity> PrepareActivities(PVIMSDbContext context)
        {
            ...
            if (!context.Activities.Any(a => a.WorkFlow.Id == spontaneousWorkFlow.Id && a.QualifiedName == "ADRM"))
            {
                activities.Add(new Activity { QualifiedName = "ADRM", WorkFlow = spontaneousWorkFlow, ActivityType = ActivityTypes.UserDrivenActivity });
            }
            if (!context.Activities.Any(a => a.WorkFlow.Id == spontaneousWorkFlow.Id && a.QualifiedName == "TSC"))
            {
                activities.Add(new Activity { QualifiedName = "TSC", WorkFlow = spontaneousWorkFlow, ActivityType = ActivityTypes.UserDrivenActivity });
            }
            if (!context.Activities.Any(a => a.WorkFlow.Id == spontaneousWorkFlow.Id && a.QualifiedName == "ADRAC"))
            {
                activities.Add(new Activity { QualifiedName = "ADRAC", WorkFlow = spontaneousWorkFlow, ActivityType = ActivityTypes.UserDrivenActivity });
            }
            if (!context.Activities.Any(a => a.WorkFlow.Id == spontaneousWorkFlow.Id && a.QualifiedName == "VIGIFLOW"))
            {
                activities.Add(new Activity { QualifiedName = "VIGIFLOW", WorkFlow = spontaneousWorkFlow, ActivityType = ActivityTypes.UserDrivenActivity });
            }
            return activities;
        }
        ...
        private static IEnumerable<ActivityExecutionStatus> PrepareActivityExecutionStatusesForSpontaneousSurveilliance(PVIMSDbContext context)
        {
            ...
            var activity = context.Activities.Single(a => a.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.QualifiedName == "ADRM");
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "ADRM" && a.Description == "UNCONFIRMED"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "UNCONFIRMED", Activity = activity, FriendlyDescription = "Report submitted for confirmation" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "ADRM" && a.Description == "CONFIRMED"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "CONFIRMED", Activity = activity, FriendlyDescription = "Report confirmed by technician" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "ADRM" && a.Description == "DELETED"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "DELETED", Activity = activity, FriendlyDescription = "Report deleted by technician" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "ADRM" && a.Description == "MEDDRASET"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "MEDDRASET", Activity = activity, FriendlyDescription = "MedDRA term set by technician" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "ADRM" && a.Description == "CLASSIFICATIONSET"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "CLASSIFICATIONSET", Activity = activity, FriendlyDescription = "Report classified by technician" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "ADRM" && a.Description == "CAUSALITYSET"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "CAUSALITYSET", Activity = activity, FriendlyDescription = "Causality set by technician" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "ADRM" && a.Description == "CAUSALITYCONFIRMED"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "CAUSALITYCONFIRMED", Activity = activity, FriendlyDescription = "Causality confirmed by technician" });
            }

            var meddraActivity = context.Activities.Single(a => a.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.QualifiedName == "TSC");
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "TSC" && a.Description == "CAUSALITYSET"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "CAUSALITYSET", Activity = meddraActivity, FriendlyDescription = "Causality set by technician" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "TSC" && a.Description == "CAUSALITYCONFIRMED"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "CAUSALITYCONFIRMED", Activity = meddraActivity, FriendlyDescription = "Causality confirmed by technician" });
            }

            var adracActivity = context.Activities.Single(a => a.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.QualifiedName == "ADRAC");
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "ADRAC" && a.Description == "CAUSALITYSET"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "CAUSALITYSET", Activity = adracActivity, FriendlyDescription = "Causality set by technician" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "ADRAC" && a.Description == "CAUSALITYCONFIRMED"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "CAUSALITYCONFIRMED", Activity = adracActivity, FriendlyDescription = "Causality confirmed by technician" });
            }

            var e2bActivity = context.Activities.Single(a => a.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.QualifiedName == "Vigiflow");
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "Vigiflow" && a.Description == "NOTGENERATED"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "NOTGENERATED", Activity = e2bActivity, FriendlyDescription = "Report ready for Vigiflow submission" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "Vigiflow" && a.Description == "E2BINITIATED"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "E2BINITIATED", Activity = e2bActivity, FriendlyDescription = "Vigiflow data generated for report" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "Vigiflow" && a.Description == "E2BGENERATED"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "E2BGENERATED", Activity = e2bActivity, FriendlyDescription = "Vigiflow report generated" });
            }
            if (!context.ActivityExecutionStatuses.Any(a => a.Activity.WorkFlow.Description == "New Spontaneous Surveilliance Report" && a.Activity.QualifiedName == "Vigiflow" && a.Description == "E2BSUBMITTED"))
            {
                activityExecutionStatuses.Add(new ActivityExecutionStatus { Description = "E2BSUBMITTED", Activity = e2bActivity, FriendlyDescription = "Vigiflow report submitted" });
            }
