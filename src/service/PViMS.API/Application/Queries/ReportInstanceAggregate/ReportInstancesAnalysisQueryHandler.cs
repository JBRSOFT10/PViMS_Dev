...
namespace PVIMS.API.Application.Queries.ReportInstanceAggregate
{
        ...
        : IRequestHandler<ReportInstancesAnalysisQuery, LinkedCollectionResourceWrapperDto<ReportInstanceDetailDto>>
    {
        ...
        private async Task CreateLinksAsync(ReportInstance reportInstanceFromRepo, ReportInstanceDetailDto mappedReportInstance)
        {
            ...
            switch (reportInstanceFromRepo.CurrentActivity.QualifiedName)
            {
                case "ADRM":
                    ...
                    //await CreateLinksForTerminologyStepAsync(reportInstanceFromRepo, mappedReportInstance); //comment out for test
                    break;
        ...
        {
            var config = _configRepository.Get(c => c.ConfigType == ConfigType.AssessmentScale);
            if (config == null)
            {
                return;
            };
            if (reportInstanceFromRepo.CurrentActivity.CurrentStatus.Description == "Waiting for checking"
                && reportInstanceFromRepo.ReportClassificationId == 1)
            {
            ...
            else if (reportInstanceFromRepo.CurrentActivity.CurrentStatus.Description == "Checked" || reportInstanceFromRepo.CurrentActivity.CurrentStatus.Description == "Complete"
               || reportInstanceFromRepo.CurrentActivity.CurrentStatus.Description == "Incomplete" && reportInstanceFromRepo.ReportClassificationId==1)
            {
                mappedReportInstance.Links.Add(new LinkDto(_linkGeneratorService.CreateResourceUriForReportInstance("UpdateReportInstanceStatus",
                    reportInstanceFromRepo.WorkFlow.WorkFlowGuid, mappedReportInstance.Id), "delete", "PUT"));
            ...
            else if (reportInstanceFromRepo.CurrentActivity.CurrentStatus.Description == "CLASSIFICATIONSET"
                && reportInstanceFromRepo.ReportClassificationId == 2)
            {
                mappedReportInstance.Links.Add(new LinkDto(_linkGeneratorService.CreateResourceUriForReportInstance("UpdateReportInstanceStatus",
                    reportInstanceFromRepo.WorkFlow.WorkFlowGuid, mappedReportInstance.Id), "delete", "PUT"));
            ...
            else if (reportInstanceFromRepo.CurrentActivity.CurrentStatus.Description == "CAUSALITYSET"
                 && reportInstanceFromRepo.ReportClassificationId == 2)
            {
                mappedReportInstance.Links.Add(new LinkDto(_linkGeneratorService.CreateResourceUriForReportInstance("UpdateReportInstanceStatus",
                    reportInstanceFromRepo.WorkFlow.WorkFlowGuid, mappedReportInstance.Id), "delete", "PUT"));
