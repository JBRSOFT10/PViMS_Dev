...
namespace PVIMS.API.Controllers
{
    ...
    public class ReportInstancesController : ControllerBase
    {
            ...
            [FromQuery] ReportInstanceActivityResourceParameters reportInstanceResourceParameters)
        {
            ...
            {
                if(item.QualifiedName=="ADRM" &&(item.CurrentStatus== "Waiting for checking" || item.CurrentStatus == "Checked" || item.CurrentStatus == "Complete" || item.CurrentStatus == "Incomplete"))
                {
                    item.CurrentStatus=item.CurrentStatus.ToString();
                    
                }
                else
                {
                    item.CurrentStatus = item.CurrentStatus.ToString()+"("+item.ReportClassification+")";
                }
            }
