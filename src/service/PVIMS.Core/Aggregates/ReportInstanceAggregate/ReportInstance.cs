...
namespace PVIMS.Core.Aggregates.ReportInstanceAggregate
{
        ...
        : AuditedEntityBase, IAggregateRoot
    {
        ...
        public string ReportingType { get; private set; }
        ...
        public ReportInstance(WorkFlow workFlow, User currentUser, Guid contextGuid, string patientIdentifier, string sourceIdentifier,string gender, string organization, string reportingType, string facilityIdentifier)
		{
            _activities = new List<ActivityInstance>();
            _medications = new List<ReportInstanceMedication>();
            _tasks = new List<ReportInstanceTask>();
            ...
            ReportClassificationId = ReportClassification.SERIOUS.Id;
        ...
        public ActivityExecutionStatusEvent ExecuteNewEventForActivity(ActivityExecutionStatus newExecutionStatus, User currentUser, string comments, DateTime? contextDate, string contextCode,int reportClassificationId)
        {
            ...
            switch (newExecutionStatus.Description)
            {
                case "COMPLETE":
                    ...
                    if (newExecutionStatus.Activity.QualifiedName!="ADRM")
                    {
                        MoveToNextActivity(WorkFlow, currentUser);
                    }               
                    break;

                case "CLASSIFICATIONSET":
                   
                    if (newExecutionStatus.Activity.QualifiedName == "ADRM" && reportClassificationId!=2)
                    {
                        MoveToNextActivity(WorkFlow, currentUser);
                    }
                    break;
        ...
        private void MoveToNextActivity(WorkFlow workFlow, User currentUser)
        {
            ...
            switch (CurrentActivity.QualifiedName)
            {
                case "ADRM":
                    ...
                    var meddraActivity = workFlow.Activities.Single(a => a.QualifiedName == "TSC");
                ...
                case "TSC":
                    CurrentActivity.SetToOld();
                    var adracActivity = workFlow.Activities.Single(a => a.QualifiedName == "ADRAC");
                    var adracActivityInstance = new ActivityInstance(adracActivity, currentUser);
                    _activities.Add(adracActivityInstance);
                    break;

                case "ADRAC":
                    CurrentActivity.SetToOld();
                    var e2bActivity = workFlow.Activities.Single(a => a.QualifiedName == "VIGIFLOW");
