...
namespace PVIMS.Core.Aggregates.DatasetAggregate
{
    public class DatasetInstance : AuditedEntityBase
    {
        ...
        public DatasetInstance(Dataset dataset, int contextId, string tag, EncounterTypeWorkPlan encounterTypeWorkPlan)
        {
            ...
            Status = DatasetInstanceStatus.INCOMPLETED;
        ...
        {
            Status = DatasetInstanceStatus.COMPLETED;
        }
        ...
        public string GetInstanceValue(string datasetElementName)
        {
            ...
            else if (datasetElementName == "Enter source of reporting")
            {

                var instanceValue1 = DatasetInstanceValues.SingleOrDefault(div => div.DatasetElement.Id == 300);

                //var instanceValue1 = DatasetInstanceValues.SingleOrDefault(div => div.DatasetElement.FieldId == 368);

                return instanceValue1?.InstanceValue;

                //return instanceValue?.InstanceValue;


            }
            else if (datasetElementName == "Report Type")
            {
                
                var instanceValue1 = DatasetInstanceValues.SingleOrDefault(div => div.DatasetElement.Id == 303);
        ...
        public DatasetInstanceValue SetInstanceValue(DatasetElement datasetElement, string instanceValue)
        {
            ...
            if (datasetInstanceValue == null)
            {
                ...
                {
                    //if (datasetElement.Id == 112)
                    //{
                    //    var value = Regex.Replace(instanceValue, @"(\[|""|\])", string.Empty);
                    //    datasetInstanceValue = AddInstanceValue(datasetElement, value);
                    //}
                    //else
                    //{
